...
export default function DashboardLayout() {
  ...
  if (activeSection === 'powerbi') activeSection = 'powerbi';
    ...
    <div className="flex h-screen overflow-hidden bg-ivory dark:bg-noir-950 transition-colors duration-300">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        <motion.header
          ...
          className="flex-shrink-0 border-b border-[#DDD8CF] dark:border-glass-border bg-white/90 dark:bg-noir-950/80 backdrop-blur-sm px-6 py-4 z-10 flex justify-between items-center transition-colors duration-300"
        >
          {location.pathname === '/dashboard' ? (
            ...
            <div className="flex-1 flex justify-between">
              <div className="flex items-center gap-2 text-gray-500 dark:text-text-muted text-xs">
                <button
                  onClick={() => navigate('/')}
                  className="flex items-center gap-1 hover:text-uib-red transition-colors duration-200 font-medium"
